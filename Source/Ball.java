 ...
 * The bouncing ball.
 * 
 * @author Hock-Chuan Chua modified by Jason P'ng
 * @version June 2015
 */
public class Ball {
   ...
   public static final Color A_COLOR = new Color (128,21,21);
    ...
    */
   public void intersect(ContainerBox box, float timeLimit,boolean type) {
     ...
     if (type == ContainerBox.OUTER){
      CollisionPhysics.pointIntersectsRectangleOuter(
       ...
       CollisionPhysics.pointIntersectsRectangleInner(
            this.x, this.y, this.speedX, this.speedY, this.radius,
            box.minX, box.minY, box.maxX, box.maxY,
            timeLimit, tempResponse);
     }
   ...
   public void intersect(Ball another, float timeLimit) {
      ...
      CollisionPhysics.pointIntersectsMovingPoint( //threw "Exception in thread "Thread-3" java.lang.NullPointerException"
            this.x, this.y, this.speedX, this.speedY, this.radius,
   ...
   public float getKineticEnergy() {
      return 0.5f * getMass() * (speedX * speedX + speedY * speedY);//multiply by 2 so the energy is actually significant
   }
