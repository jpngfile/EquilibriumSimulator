...
public class CollisionPhysics {
   ...
   /**
    * Detect collision for a moving point bouncing outside a rectangular container,
    * within the given timeLimit.
    * If collision is detected within the timeLimit, compute collision time and 
    * response in the given CollisionResponse object. Otherwise, set collision time
    * to infinity.
   ...
   public static void pointIntersectsRectangleInner(
      ...
      assert !((pointX >= rectX1 + radius) && (pointX <= rectX2 - radius) 
            && (pointY >= rectY1 + radius) && (pointY <= rectY2 - radius))
            : "Point (with radius) is inside the rectangular container!";
      assert (radius >= 0) : "Negative radius!";
      assert (timeLimit > 0) : "Non-positive time";
      ...
      // Right border
      pointIntersectsLineSegmentNoEndPoints(pointX, pointY, speedX, speedY, radius,
            rectX2,rectY1,rectX2,rectY2, timeLimit, tempResponse);
      if (tempResponse.t < response.t) {
         response.copy(tempResponse);  // Copy into resultant response
      }
      // Left border
      pointIntersectsLineSegmentNoEndPoints(pointX, pointY, speedX, speedY, radius,
            rectX1,rectY1,rectX1,rectY2, timeLimit, tempResponse);
      if (tempResponse.t < response.t) {
         response.copy(tempResponse);  // Copy into resultant response
      }
      // Top border
      pointIntersectsLineSegmentNoEndPoints(pointX, pointY, speedX, speedY, radius,
            rectX1,rectY1,rectX2,rectY1, timeLimit, tempResponse);
      if (tempResponse.t < response.t) {
         response.copy(tempResponse);  // Copy into resultant response
      }
      // Bottom border
      pointIntersectsLineSegmentNoEndPoints(pointX, pointY, speedX, speedY, radius,
            rectX1,rectY2,rectX2,rectY2, timeLimit, tempResponse);
      if (tempResponse.t < response.t) {
         response.copy(tempResponse);  // Copy into resultant response
      }
      
      //intersect the points
      pointIntersectsPoint (pointX,pointY,speedX,speedY,radius,rectX1,rectY1,0,timeLimit,tempResponse);
      if (tempResponse.t < response.t) {
         response.copy(tempResponse);  // Copy into resultant response
      }
      pointIntersectsPoint (pointX,pointY,speedX,speedY,radius,rectX2,rectY1,0,timeLimit,tempResponse);
      if (tempResponse.t < response.t) {
         response.copy(tempResponse);  // Copy into resultant response
      }
      pointIntersectsPoint (pointX,pointY,speedX,speedY,radius,rectX1,rectY2,0,timeLimit,tempResponse);
      if (tempResponse.t < response.t) {
         response.copy(tempResponse);  // Copy into resultant response
      }
      pointIntersectsPoint (pointX,pointY,speedX,speedY,radius,rectX2,rectY2,0,timeLimit,tempResponse);
      if (tempResponse.t < response.t) {
         response.copy(tempResponse);  // Copy into resultant response
      }
